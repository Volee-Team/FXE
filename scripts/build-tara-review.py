...
copy changes, run this, and republish the artifact (see docs/copy-review.md).
    ...
    python3 scripts/build-tara-review.py
"""
...
def main():
    ...
    page = (TEMPLATE.replace("@@WORDS@@", words).replace("@@QUESTIONS@@", qs).replace("@@TASKS@@", ts)
            .replace("@@DATA@@", json.dumps(data)).replace("@@TOTAL@@", str(total)).replace("@@NQ@@", str(len(QUESTIONS))))
    out = pathlib.Path(__file__).resolve().parent.parent / "docs" / "tara-review" / "index.html"
    out.write_text(page)
