...
| 10 | ~~**Backup artifacts are readable by any signed-in GitHub user**~~ | [Alex] then [me] | **done 2026-09-22.** `backup.yml` encrypts with `age` to the public key in `.github/backup-recipient.txt` (Alex's `age1…`, derived on 2026-09-22 with `age-keygen -y` from the key file in his home folder; the private half is in LastPass and never in the repo). The 11 old unencrypted artifacts were deleted 2026-09-18. First encrypted run 2026-09-22 (run 35691530447, 93 KB): downloaded, decrypted with his key, gunzipped, a real `pg_dump` with 47 tables, `auth.users` and `supabase_migrations`. Next restore drill: 2026-10-12 (D4) |
...
|---|---|---|---|
| SQL probes (rules, privileges, attacks, concurrency) | every PR, and locally | 555 checks, 25 probes | none known |
