...
* **After a compaction, the summary is a claim, not a record.** The session-start hook replays Alex's last three prompts verbatim from the prompt log and the first lines of the last reply. Read them, then `git log --oneline -5`, the top of `docs/whats-next.md` and the newest changelog entry, before touching code. Where the summary and the repo disagree, the repo wins. (2026-09-23; a long session is compacted, never cleared, so this happens several times a day.)
